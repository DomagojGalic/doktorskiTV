...
\begin{proof}
    ...
    Po korolaru \ref{kor:2.8} i po teoremu \ref{tm:2.11} postoji jedinstveno pre\v sirenje $\masP_F$ koje je vjerojatnost na $\borel{\real}$.
...
\begin{zad} \label{zad:5.10}
    Opi\v site korespondenciju kao u \eqref{jed:5.9} na $\urePar{\real}{\borel{\real}}$i poka\v zite da je korespondencija u \eqref{jed:5.9} poseban slu\v caj koresponedencije na $\extReal$.
\end{zad}
...
\begin{defn}    \label{defn:5.11-1}
    Slu\v cajna varijabla je $X$ \emph{diskretna} ako postoji prebrojiv skup $S$ takva da je
    \begin{equation*}
        X \in S \; (g.s.),
    \end{equation*}
...
Osim prvog momenta slu\v cajen varijable, kojeg jo\v s nazivamo i o\v cekivanjem, va\v zan je i drugi moment to jest \emph{drugi centrali moment} $\masE [ (X - \masE X)^2 ]$, kojeg nazivamo \emph{varijancom}.
